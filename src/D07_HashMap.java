...
public class D07_HashMap {
	...
	public static void main(String[] args) {
		...
		map1.put("나이","5살");
		map1.put("견종","포메라니안");
		map1.put("견종", "치와와"); // 같은 키 값을 이용하면 값이 수정된다
		map1.put("주인의 나이", "5살");
		...
		// # 반복문으로 Map 활용하기
		//	- keySet	: Key들로 이루어진 Set을 반환한다 	-> key는 중복값이 없기 때문에 Set
		//	- values	: Value들로 이루어진 Collection을 반환한다 - > value는 중복값이 있을 수 있기 때문에 Collection
		//	- entrySet	: Entry<K, B>들로 이루어진 Set을 반환한다
		
		System.out.println("map1의 key들 : " + map1.keySet());
		System.out.println("map1의 values들 : " + map1.values());
		...
		// # values로 돌리기
		//	- Key로 Value를 찾는것은 쉽지만 Value로 Key를 찾는 것은 불가능하다
		...
		// # containsKey
		// 입력한 값이 키에 존재하는지를 boolean타입으로 반환 -> 정확하게 입력해야 한다
		...
		//	- Key와 Value모두를 가지고 지우는 경우 : 정확하게 값도 일치할 때 삭제한다 -> boolean타입으로 반환
		...
		// # getOrDefault 메서드를 이용해 null값이 반환되는 상황이라면 설정한 기본값을 반환받을 수 있다 
		...
		// 새로운 해쉬맵 인스턴스를 값으로 저장, 키값으로 해당 해쉬맵을 꺼낼 수 있게 된다
		...
		// 중복되는 값은 키값으로 사용할 수 없기 때문에 키값에는 전화번호같은 값을 사용하는 것이 좋다
