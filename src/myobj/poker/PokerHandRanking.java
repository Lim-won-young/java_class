...
public class PokerHandRanking {
	...
	static {
		...
		Collections.addAll(MOUNTAIN,Rank.TEN, Rank.JACK, Rank.QUEEN, Rank.KING, Rank.ACE);
	...
	public PokerHandRanking(PokerHand hands) {
		rankCount = new ArrayList<>(Rank.NUM_OF_RANKS);
		suitCount = new ArrayList<>(Suit.NUM_OF_SUIT);
		
		for (int i = 0; i < Rank.NUM_OF_RANKS; i++) {
			rankCount.add(0);
		}
	...
	public void check(PokerHand player) {
		...
		if (suitCount.contains(5) && checkMountain(ranks)) {
			...
			// indexOF : 앞에서부터 해당 요소를 찾는다
			// lastIndexOF : 뒤에서부터 해당 요소를 찾는다
